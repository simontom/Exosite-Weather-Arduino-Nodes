...
void initRadio(void) {
	if ( !manager.init() ) {
		...
		driver.setTxPower(RADIO_POWER_LOWER);
			...
			manager.addRouteTo(SINK_NODE_ADDR, BATHROOM_NODE_ADDR);
			//manager.addRouteTo(OUTSIDE_NODE_ADDR, VERANDA_NODE_ADDR);
			...
			//manager.addRouteTo(LIGHT_NODE_ADDR, LIGHT_NODE_ADDR);
		#endif
	}
}
